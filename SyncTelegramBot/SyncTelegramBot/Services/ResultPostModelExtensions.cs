...
public static class ResultPostModelExtensions
{
        ...
        await Matcher(handlerResult, async handler =>
        {
            ...
            //return await splitContragent.Match(async splitArray =>
                //{
                    ...
                    handler.Model.Contragent = handler.PostFromBotModel.Contragent;
                    return await Task.FromResult(handlerResult);
                // },
                // failure =>
                // {
    ...
    public static async Task<Result<DataForRequest, ValidationException>> HandleDecryptionContractAsync(
        ...
        { 
            ...
            handler.Model.Decryption[0].Contract = handler.PostFromBotModel.ContractFromDecryptionOfPayment;
    ...
    public static async Task<Result<DataForRequest, ValidationException>> HandleDecryptionDocumentAsync(
        ...
        await Matcher(handlerResult, async handler =>
        {
            ...
            //         return Task.FromResult(handlerResult);
            //     });
            var document = handler.PostFromBotModel.DocumentFromDecryptionOfPayment!.Split('_');
            var typeInvoice = "";
            if (document[1] == "Накладная")
                typeInvoice = handler.PostFromBotModel.Type is PostType.Receive ? "Приходная" : "Расходная";
            handler.Model.Decryption ??= new DecryptionPayment[] { new() { LineNumber = "1" } };
            handler.Model.Decryption[0].DocumentType = $"StandardODATA.Document_{typeInvoice+document[1]}";
            handler.Model.Decryption[0].Document = document[0];
            return await Task.FromResult(handlerResult);
        });
    ...
    public static async Task<Result<DataForRequest, ValidationException>> HandleCorrespondenceAsync(
        ...
        await Matcher(handlerResult, async handler =>
        {
            ...
            //         return Task.FromResult(handlerResult);
            //     });
            handler.Model.Correspondence = handler.PostFromBotModel.Correspondence;
            return await Task.FromResult(handlerResult);
        });
        ...
        this Result<DataForRequest, ValidationException> handlerResult, string ownerParameter) => await Matcher(handlerResult, async handler =>
    {
        ...
        //         return Task.FromResult(handlerResult);
        //     });
        handler.Model.LoanAgreement = handler.PostFromBotModel.LoanAgreement;
        return await Task.FromResult(handlerResult);
    });
        ...
        this Result<DataForRequest, ValidationException> handlerResult) => await Matcher(handlerResult, async handler =>
    {
        ...
        //         return Task.FromResult(handlerResult);
        //     });
        handler.Model.Employee = handler.PostFromBotModel.Employee;
        return await Task.FromResult(handlerResult);
    });
        ...
        this Result<DataForRequest, ValidationException> handlerResult) => await Matcher(handlerResult, async handler =>
    {
        ...
        // return await splitCurrency.Match(async splitCurrencyArray =>
            // {
                ...
                //     return handlerResult ;
                // }
                handler.Model.Currency = handler.PostFromBotModel.Currency;
        ...
        this Result<DataForRequest, ValidationException> handlerResult) => await Matcher(handlerResult, async handler =>
    {
        ...
        //         return Task.FromResult(handlerResult);
        //     });
        handler.Model.Organisation = handler.PostFromBotModel.Organisation;
        return await Task.FromResult(handlerResult);
    });
        ...
        this Result<DataForRequest, ValidationException> handlerResult) => await Matcher(handlerResult, async handler =>
    {
        ...
        //         return Task.FromResult(handlerResult);
        //     });
        handler.Model.OrganisationAccount = handler.PostFromBotModel.OrganisationAccount;
        return await Task.FromResult(handlerResult);
    });
        ...
        this Result<DataForRequest, ValidationException> handlerResult, string ownerParameter) => await Matcher(handlerResult, async handler =>
    {
        ...
        //     }
        //
        //     handler.Model.ContractType = $"StandardODATA.Document_Расход{splitContractArray[3]}";
        //     
        //     return handlerResult;
        // },
        // failure =>
        ...
        //     return Task.FromResult(handlerResult );
        // });
        var expense = handler.PostFromBotModel.Contract!.Split('_');
        handler.Model.Contract = expense[0];
        expense[1] = expense[1] == "Касса" ? "ИзКассы" : "СоСчета";
        handler.Model.ContractType = $"StandardODATA.Document_Расход{expense[1]}";
        return await Task.FromResult(handlerResult);
    });
        ...
        this Result<DataForRequest, ValidationException> handlerResult) => await Matcher(handlerResult, async handler =>
    {
        ...
        //     });
        handler.Model.Decryption ??= new DecryptionPayment[] { new() { LineNumber = "1" } };
        handler.Model.Decryption[0].VatRate = handler.PostFromBotModel.VatRate;
        return await Task.FromResult(handlerResult);
    });
        ...
        this Result<DataForRequest, ValidationException> handlerResult) => await Matcher(handlerResult, async handler =>
    {
        ...
        //         return Task.FromResult(handlerResult);
        //     });
        handler.Model.TypeOfTax = handler.PostFromBotModel.TypeOfTax;
        return await Task.FromResult(handlerResult);
    });
